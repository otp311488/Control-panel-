...
// Parse M3U content and return an array of channels with only channelName and channelLogoUrl
function parseM3UContent($content) {
    ...
    foreach ($lines as $line) {
        ...
        if (strpos($line, '#EXTINF:') === 0) {
            ...
            // Extract channelName and channelLogoUrl
            preg_match_all('/(\w+)\s*=\s*("([^"]*)"|[^,\s]+)/', $line, $matches, PREG_SET_ORDER);
            foreach ($matches as $match) {
                $key = trim($match[1]);
                $value = isset($match[3]) ? trim($match[3]) : trim($match[2]);

                // Map channelName and channelLogoUrl directly
                if ($key === "channelName") {
                    $channel["channelName"] = $value;
                } elseif ($key === "channelLogoUrl") {
            ...
            // Add the channel to the array immediately after parsing #EXTINF
            $channels[] = $channel;
